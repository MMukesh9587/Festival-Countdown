...
export function ShareCanvas({ festival, customMessage, onImageGenerated }: ShareCanvasProps) {
  ...
  useEffect(() => {
    const draw = () => {
      ...
      bg.onerror = (e) => {
          console.error("Failed to load image for canvas. Drawing with fallback background.", e);
          drawContent(); 
      };
      
      // Use a proxy for external images to avoid CORS issues
      if (imageUrl.startsWith('http') && !imageUrl.includes('unsplash.com')) {
          bg.src = `/_next/image?url=${encodeURIComponent(imageUrl)}&w=1200&q=75`;
      } else {
          bg.src = imageUrl;
      }
    };
  ...
  }, [festival, customMessage, language, onImageGenerated, t]);
